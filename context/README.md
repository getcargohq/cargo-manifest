...
| `motion/` | GTM motions: trigger, sequence, owner, metric (the implementation lives in `infra/`) |
| `client/` | Accounts of record: won, lost, and why |
| `insight/` | Durable observations that do not fit elsewhere yet |

## Motion vs play

A **motion** is the go-to-market move (the narrative: trigger, sequence, owner,
