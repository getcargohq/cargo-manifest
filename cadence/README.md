...
The `cadence` skill runs all three modes.

- **Monday: plan the week.** It pulls from `plan/`
...
- **Daily: log.** What happened, what moved, what is
  now stuck. Two minutes, not a status report.
- **Friday: review.** It digests `outputs/` into
...
- **Carryover is the honesty mechanism.** An item that slips three weeks in a
  row is not a task, it is a decision you are avoiding. The Monday plan surfaces
  it as such.
